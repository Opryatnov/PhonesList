...
@IBOutlet weak var phonePhoto: UIImageView!
    ...
    class func configureView(model: PhoneModel) -> PhoneInfoView {
        let phoneView = UINib(nibName: String(describing: PhoneInfoView.self),
                              bundle: .main).instantiate(withOwner: nil, options: nil).first as! PhoneInfoView
